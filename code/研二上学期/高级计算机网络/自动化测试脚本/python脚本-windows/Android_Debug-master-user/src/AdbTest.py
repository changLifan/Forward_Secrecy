...
# android目录
ANDROID_HOME = "D:\\software\\androidSDK\\sdk\\platform-tools"
# adb路径
_adb = ""
# pyadb
_pyadb = None
...
class AdbService(threading.Thread):
    ...
    def run(self):
        ...
        child = pexpect.popen_spawn.PopenSpawn("adb shell getevent -t", timeout=120)
...
if(ANDROID_HOME != None):
    ...
    print("你连接的设备是：",_pyadb.get_devices())
    ...
    numIndex = int(input("输入回放次数(请输入正整数): "))
    for num in range(0, numIndex):
        __play = True
        flag = input("输入y开始回放（输入其他字符或者ENTER键，将会结束回放）: ")
